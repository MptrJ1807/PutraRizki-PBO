...
class FormRumahsakit:
    ...
    def aturKomponen(self):
        ...
        Cbo['values'] = ('Jantung','Mata','Tulang','Gigi','Kulit','Paru-Paru','Anak Kecil')
        Cbo.current()      
        ...
        # define columns
        columns = ('iddktr', 'nip', 'nama','jenis_kelamin','spesialis')

        self.tree = ttk.Treeview(mainFrame, columns=columns, show='headings')
        # define headings
        self.tree.heading('iddktr', text='ID')
        self.tree.column('iddktr', width="30")
        self.tree.heading('nip', text='NIP')
        self.tree.column('nip', width="60")
        self.tree.heading('nama', text='Nama')
        self.tree.column('nama', width="200")
        self.tree.heading('jenis_kelamin', text='JK')
    ...
    def TampilkanData(self, event=None):
        ...
        res = rs.getByNIP(nip)
        self.txtNama.delete(0,END)
        self.txtNama.insert(END,rs.nama)
        jenis_kelamin = rs.jk
        if(jenis_kelamin=="P"):
            self.P.select()
        else:
            self.L.select()
        self.txtSpesialis.set(rs.spesialis)   
        self.btnSimpan.config(text="Update")
                 
    def onSimpan(self, event=None):
        nip = self.txtNIP.get()
        nama = self.txtNama.get()
        jenis_kelamin = self.txtJK.get()
        spesialis = self.txtSpesialis.get()
        
        rs = Rumahsakit()
        rs.nip = nip
        rs.nama = nama
        rs.jenis_kelamin = jenis_kelamin
        rs.spesialis = spesialis
        if(self.ditemukan==True):
            res = rs.updateByNIP(nip)
            ket = 'Diperbarui'
        else:
            res = rs.simpan()
            ket = 'Disimpan'
...
if __name__ == '__main__':
    root = tk.Tk()
    aplikasi = FormRumahsakit(root, "Aplikasi Data Dokter")
    root.mainloop() 